...
def main():
    ...
    for i, method in enumerate(Methods):
        ...
        for col, input_type in enumerate(["random", "sin"]):
            ...
            mse_per_timestep = np.mean(
                (all_preds[:, :, :] - all_labels_x[:, :, :]) ** 2, axis=(0, 2)
            )  # 输出形状为 (200,)
            dis_per_timestep = np.mean(
                (all_preds[:, :, :3] - all_labels_x[:, :, :3]) ** 2, axis=(0, 2)
            )  # 输出形状为 (200,)
            angle_per_timestep = np.mean(
                (all_preds[:, :, 3:] - all_labels_x[:, :, 3:]) ** 2, axis=(0, 2)
            )  # 输出形状为 (200,)
            ...
            axes[0, col].plot(
                steps[0:horizen],
                np.log10(dis_per_timestep[start_id : start_id + horizen]),
                color=Color[i],
    ...
    for row in range(3):
        ...
        axes[row, -1].legend(
            handles, labels, loc="center left", bbox_to_anchor=(1.05, 0.5), borderaxespad=0, fontsize=16
        )
    plt.tight_layout()
    fig.savefig("./results/" + args.env + "/" + args.suffix + "/plot.png", dpi=300)
    plt.show()
    plt.close(fig)

    loss_dir = "./results/" + args.env + "/" + args.suffix + "/" + "IBKN" + "/" + "train_losses.json"
    # 读取 JSON 文件
    ...
    plt.tight_layout()
    plt.savefig("./results/" + args.env + "/" + args.suffix + "/loss.png", dpi=300)
    plt.show()
