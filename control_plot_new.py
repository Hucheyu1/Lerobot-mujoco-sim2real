...
if __name__ == "__main__":
    ...
    if scenario == 'noise_robustness':
            ...
            for idx, traj_name in enumerate(['Fig8', 'FigStar']):
                ...
                ax0.plot(ref_traj[:, 0], ref_traj[:, 1], ref_traj[:, 2],
                        color='black', linewidth=2.5, alpha=1.0, label='Reference', zorder=1)
                ...
                for method in method_config:
                    ...
                    for rob_type in ['none', 'KEM']:
                        ...
                        # 绘制 3D 轨迹 (Fig 1)
                        ax0.plot(pre_traj[::5, 0], pre_traj[::5, 1], pre_traj[::5, 2],
                                color=rob_cfg['color'], alpha=rob_cfg['alpha'], linewidth=1.5,
                                linestyle=rob_cfg['line_style'], label=rob_cfg['name'], zorder=2)
                        
                        if rob_type == 'none':
                            # 绘制误差 (Fig 2 - row 0)
                            ax_err.plot(steps_pre[::timestep2], error[::timestep2, 0], color='red', linestyle='--', label='error x (None)')
                            ax_err.plot(steps_pre[::timestep2], error[::timestep2, 1], color='darkorange', linestyle='--', label='error y (None)')
                            ax_err.plot(steps_pre[::timestep2], error[::timestep2, 2], color='brown', linestyle='--', label='error z (None)')
                        
                        if rob_type == 'KEM':
                            all_labels_u = results[traj_name][method][rob_type]['u']
                            # 绘制位置分量 (Fig 2 - row 1)
                            ax_pos.plot(steps_pre[::timestep1], pre_traj[::timestep1, 0], color='blue', linestyle=':', label='Pred x (KEM)')
                            ax_pos.plot(steps_pre[::timestep1], pre_traj[::timestep1, 1], color='purple', linestyle=':', label='Pred y (KEM)')
                            ax_pos.plot(steps_pre[::timestep1], pre_traj[::timestep1, 2], color='black', linestyle=':', label='Pred z (KEM)')
                            
                            # 绘制误差 (Fig 2 - row 0)
                            ax_err.plot(steps_pre[::timestep2], error[::timestep2, 0], color='blue', label='error x (KEM)')
                            ax_err.plot(steps_pre[::timestep2], error[::timestep2, 1], color='purple', label='error y (KEM)')
                            ax_err.plot(steps_pre[::timestep2], error[::timestep2, 2], color='black', label='error z (KEM)')
                            
                            # 绘制输入 (Fig 2 - row 2)
                            for j in range(min(7, all_labels_u.shape[1])):
                                ax_input.plot(steps_pre[::timestep1], all_labels_u[::timestep1, j], color=Color[j], label=f'ω{j+1}')
            ...
            axes1[0, -1].legend(by_label1.values(), by_label1.keys(), loc="center left", bbox_to_anchor=(1.05, 0.5), borderaxespad=0)
            ...
            for r in range(3):
                ...
                axes2[r, -1].legend(by_label2.values(), by_label2.keys(), loc="center left", bbox_to_anchor=(1.05, 0.5), borderaxespad=0)
