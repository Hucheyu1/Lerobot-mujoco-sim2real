...
def main():
    ...
    for row_idx, input_type in enumerate(input_types):
        ...
        # 2. 绘制误差对比 Error
        for i, method in enumerate(Methods):
            start_id = args.seq_len if (method.startswith("Koopformer") or method.startswith("KoopmanLSTMlinear")) else 1
            mse = plot_data[method]['mse']
            ax_err.plot(steps[0:151], np.log10(mse[start_id:start_id+151]), 
                        color=Color[i], label=Methods_name[i], linewidth=1.5)
        ...
        # --- 第二/四行：各轴 x, y, z 对比 ---
        ax_xyz = [fig.add_subplot(gs[base_row+1, 0:2]), 
        ...
        for axis_i in range(3):
            # 绘制真值
            ax_xyz[axis_i].plot(steps, plot_data[Methods[-1]]['labels_x'][:, axis_i], 
                                color='orange', linewidth=2, label='Ground Truth', zorder=0)
            # 绘制各方法预测值
            for i, method in enumerate(Methods):
                ax_xyz[axis_i].plot(steps, plot_data[method]['preds'][:, axis_i], 
                                    color=Color[i], linestyle='--', alpha=0.8, label=Methods_name[i])
            
            ax_xyz[axis_i].set_title(f'{input_type.upper()} - {axis_names[axis_i]} axis', fontsize=12)
            ax_xyz[axis_i].set_xlabel('Steps')
            if axis_i == 0: ax_xyz[axis_i].set_ylabel('Position (m)')
            if axis_i == 2: ax_xyz[axis_i].legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=8)
