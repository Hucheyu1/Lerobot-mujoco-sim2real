...
class Kinematics:
    ...
    def createSolver(self):
        ...
        # 获取末端执行器的ID
        self.ee_id = (
            self.model.getFrameId(self.frame_name)
            if self.frame_name == "gripperframe"
            else self.model.getFrameId(self.frame_name, pin.FrameType.BODY)
        )
        # a. 位置误差函数：末端位置 - 目标位置
        ...
        # 这是一个带权重的多目标优化
        self.opti.minimize(
            10.0 * self.translational_cost
            + 0.1 * self.rotation_cost
            + 0.0 * self.regularization_cost
