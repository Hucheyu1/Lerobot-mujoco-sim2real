...
if scenario == "noise_robustness":
    ...
    axes_map = {
        "Fig8": {
            ...
            "ts": [
                fig.add_subplot(gs[1, 0:2]),  # 第1行，X (占2列)
        ...
        "FigStar": {
            ...
            "ts": [
                fig.add_subplot(gs[2, 0:2]),  # 第2行，X (占2列)
    ...
    for traj_name in ["Fig8", "FigStar"]:
        ...
        for method in method_config:
            ...
            for rob_type in ["KEM", "ESO", "both"]:  # 或 'none',
                ...
                for dim in range(3):
                    axes_ts[dim].plot(
                        pred_steps[::timestep],
                        error[::timestep, dim] if plot_error else pre_traj[::timestep, dim],
                        color=rob_cfg["color"],
                        linestyle=rob_cfg["line_style"],
                        linewidth=1.0,
                        alpha=0.8,
        ...
        # 将图例放在 Z轴图的右侧外面
        z_ax.legend(by_label.values(), by_label.keys(), loc="center left", bbox_to_anchor=(1.02, 0.5), fontsize=14)
