...
if __name__ == "__main__":
    ...
    if scenario == 'noise_robustness':
            ...
            for method in method_config:
                ...
                for rob_type in ['none', 'KEM']:
                    ...
                    if rob_type == 'none':
                        ...
                        # 绘制误差（x/y/z）
                        ax2.plot(steps2, error[:: timestep2, 0], color='red', alpha=1,linestyle='--', linewidth=1.5, label='error x (no UKF)')
                        ax2.plot(steps2, error[:: timestep2, 1], color='darkorange', alpha=1,linestyle='--', linewidth=1.5, label='error y (no UKF)')
                        ax2.plot(steps2, error[:: timestep2, 2], color='brown', alpha=1,linestyle='--', linewidth=1.5, label='error z (no UKF)')
                    if rob_type == 'KEM':
                        # 绘制预测值（x/y/z）
                        all_labels_u = results[traj_name][method][rob_type]['u']
                        ax1.plot(steps1, pre_traj[:: timestep1, 0], color='blue', alpha=0.8,linestyle='--', linewidth=1.5, label='Pred x (with UKF)')
                        ax1.plot(steps1, pre_traj[:: timestep1, 1], color='purple', alpha=0.8,linestyle='--', linewidth=1.5, label='Pred y (with UKF)')
                        ax1.plot(steps1, pre_traj[:: timestep1, 2], color='black', alpha=0.8,linestyle='--', linewidth=1.5, label='Pred z (with UKF)') 
                        # 绘制误差（x/y/z）
                        ax2.plot(steps2, error[:: timestep2, 0], color='blue', linestyle='-', linewidth=1.5, label='error x (with UKF)')
                        ax2.plot(steps2, error[:: timestep2, 1], color='purple', linestyle='-', linewidth=1.5, label='error y (with UKF)')
                        ax2.plot(steps2, error[:: timestep2, 2], color='black', linestyle='-', linewidth=1.5, label='error z (with UKF)') 
                        # 绘制7个输入量（ω₁~ω₇）
                        for j in range(args.u_dim):
                            ax3.plot(steps1, all_labels_u[::timestep1, j], color=Color[j], linewidth=1.5, label=f'ω{j+1}')
        ...
        plt.savefig(
            os.path.join(save_Fig_path, f"{scenario}.png"), format="png",\
