...
from .utility import PatchTST_Backbone, SimpleAutoformer, Transformer_Baseline
from .base_model import StableKoopmanOperator
from .Kan import KAN


class Koopformer(StableKoopmanOperator):
    def __init__(self, x_dim: int, u_dim: int, seq_len: int, d_model: int, rho_max: float = 0.99):
        self.x_dim = x_dim
        self.u_dim = u_dim
        self.seq_len = seq_len
        self.d_model = d_model
        self.Nkoopman = d_model + x_dim 
        super().__init__(self.Nkoopman, self.u_dim)
        # --- 1. 编码器和解码器 ---
        # 状态编码器：使用 PatchTST_Backbone
        self.x_encode_net = Transformer_Baseline(
            input_dim = x_dim, 
            seq_len = seq_len, 
            d_model=d_model
        )
...
class Koopformer_KAN(Koopformer):
    def __init__(self, x_dim, u_dim, seq_len, d_model):
        super(Koopformer, self).__init__(x_dim, u_dim, seq_len, d_model)
