...
def spectral_radius_loss(model, margin=0.99):
    if hasattr(model, "lA"):
        A = model.lA.weight
...
def pred_and_eval_loss_old(
    ...
    return dict(pred=pred, pred_loss=pred_loss / cont, dis_loss=dis_loss / cont, angle_loss=angle_loss / cont)
...
def koopformer_eval_loss_new(
    ...
    return dict(pred=pred_trajectory, pred_loss=avg_pred_loss, dis_loss=avg_dis_loss, angle_loss=avg_angle_loss)
...
def koopformer_eval_loss_old(
    ...
    return dict(pred=pred, pred_loss=avg_pred_loss, dis_loss=avg_dis_loss, angle_loss=avg_angle_loss)
